...
-- Generates a random number between 1 million and 10 million.
...
-- against the efficiency of the expM with the same input.
...
-- Filters the list of infinite natural numbers larger than 1 to only contain non-primes (composites)
composites :: [Integer]
...
-- Returns the first carmichael number that fails the test (iterates under 100).
...
foolMRcarmichael = [primeMR 3 r | r <- carmichael]

foolMRTest :: Int -> IO Integer
foolMRTest 100 = foolTest 0
foolMRTest n = do
    r <- foolMRcarmichael !! n
    if r then return (carmichael !! n) else ((foolMRTest (n + 1)))
